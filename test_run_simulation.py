...
def helper_test_run_simulation(params):
    ...
    actual_frequency, actual_num_steps = run_simulation(actual_grid, R, thresholds, centers, max_steps)
    
    expected_filename = params["expected_filename"]
    expected_grid, centers, expected_num_steps = utility.read_grid(expected_filename, final=True)
    ...
    if actual_num_steps != expected_num_steps:
        msg = "Actual and expected number of steps don't match:\n"
        msg += "    Actual: {}\n"
        msg += "    Expected: {}\n"
        msg = msg.format(actual_num_steps, expected_num_steps)
        msg = msg + "\n" + recreate_msg
        pytest.fail(msg)
